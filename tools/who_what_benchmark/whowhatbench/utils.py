...
def get_chat_template_for_model(model_path, tokenizer=None):
    ...
    # Hardcoded chat template mappings by exact AutoConfig.model_type value.
    chat_template_map = {
    ...
    try:
        ...
        if model_type in chat_template_map:
            ...
            logger.info(f"Matched model_type '{model_type}' with chat template source: {template_source}")
            return template_source
        logger.info(f"No custom chat template mapping for model_type '{model_type}'")
    except Exception as e:
        logger.warning(f"Failed to infer model_type via AutoConfig for '{model_path}': {e}")
